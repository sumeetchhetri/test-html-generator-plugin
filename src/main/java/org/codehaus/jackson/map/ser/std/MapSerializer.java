    ...
    implements ResolvableSerializer
{
    ...
    {
        ObjectNode o = createSchemaNode("map", true);
         ...
         *    information.
         */
        JavaType keyContentType = null, valueContentType = null;
        if (typeHint != null) {
            if (typeHint instanceof ParameterizedType) {
                Type[] typeArgs = ((ParameterizedType) typeHint).getActualTypeArguments();
                if (typeArgs.length == 2) {
                	keyContentType = provider.constructType(typeArgs[0]);
                	valueContentType = provider.constructType(typeArgs[1]);
                }
            }
        }
        if (keyContentType != null && valueContentType!=null) {
            ...
            if (keyContentType.getRawClass() != Object.class) {
                JsonSerializer<Object> ser = provider.findValueSerializer(keyContentType, _property);
            ...
            propertiesNode.put("key", schemaNode);
            //o.put("items", schemaNode);
            
            if (valueContentType.getRawClass() != Object.class) {
                JsonSerializer<Object> ser = provider.findValueSerializer(valueContentType, _property);
                if (ser instanceof SchemaAware) {
                    schemaNode = ((SchemaAware) ser).getSchema(provider, null);
                }
            }
            ...
            propertiesNode.put("value", schemaNode);
            //o.put("items", schemaNode);
            
            o.put("types", propertiesNode);
        }
        return o;
